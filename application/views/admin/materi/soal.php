...
<script>
         ...
         "columns": [{
            ...
            },
            {
               ...
               "render": function(data, type, row, meta) {
                  ...
                  if (row.mtr_kategori == 2) {
                     btn += `<a href="<?= base_url('admin/materi/soal/') ?>` + row.mtr_slug + `" class="mr-1 btn btn-purple btn-sm"><i class="fas fa-align-justify" title="Soal"></i> Soal</a>`;
                  }
                  return btn;
               },
               className: "text-center",
   ...
   $(document).off("click", "button.soal")
      .on("click", "button.soal", function(e) {
         $('#dataSoalModal').modal('show');
         $('#dataSoalModalTitle').html('<i class="fas fa-plus-circle"></i> Tambah Soal');
         $(document).off("click", "#dataSoalModalSave").on("click", "#dataSoalModalSave", function(e) {
            simpan('form-data-soal');
         });
      });
   ...
   $(document).off("submit", "#form-data-soal")
      .on("submit", "#form-data-soal", function(e) {
         event.preventDefault();
         simpan('form-data-soal');
      });
      ...
      $.ajax({
         ...
         success: function(data) {
            if (data.status == 1) {
               ...
               $('#dataSoalModal').modal('hide');
               toastr.success(data.pesan);
               tabel.ajax.reload(null, true);
            } else if (data.status == 3) {
               $.each(data.pesan, function(i, item) {
                  $('#' + form + ' #' + i + '_error').html(item);
                  $('#' + form + ' #' + i + '_error').show();
                  if (item) {
                     $('#' + form + ' #' + i).removeClass("is-valid").addClass("is-invalid");
                  } else {
                     $('#' + form + ' #' + i).removeClass("is-invalid").addClass("is-valid");
                  }
               });
            } else {
               $('#dataModal').modal('hide');
